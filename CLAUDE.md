...
作業環境によってフローが分岐する。詳細は `.claude/rules/work-environment.md` を参照。

### ユーザーの端末内で作業する場合（基本フロー）
...
ユーザーが手元で動作確認するには `git pull` が必要なため、ブランチを切って作業し、実装後にコミット・プッシュまで完了させる。プッシュ後にユーザーへレビュー・動作確認を依頼する。
